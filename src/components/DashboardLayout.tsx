...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-background flex font-body">
      {/* SideNavBar */}
      <aside className="fixed left-0 top-0 h-screen w-64 bg-surface border-r border-outline-variant/10 p-6 flex flex-col z-50">
        <div className="mb-10 px-4">
          ...
          <p className="text-[10px] font-bold text-primary/40 uppercase tracking-widest mt-2">Family Ecosystem</p>
        </div>
      ...
      <div className="flex-1 ml-64 flex flex-col">
        {/* TopNavBar */}
        <header className="h-20 bg-surface/80 backdrop-blur-md sticky top-0 z-40 border-b border-outline-variant/10 px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-outline-variant/10">
              <div className="text-right">
                <p className="text-xs font-bold font-headline text-primary">{user.displayName || 'Parent'}</p>
                <p className="text-[10px] text-primary/40 font-bold uppercase tracking-tighter">Main Account Holder</p>
              </div>
              <img 
                src={user.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=80&h=80&fit=crop'} 
                alt="User Avatar" 
                className="w-10 h-10 rounded-xl object-cover shadow-sm border border-outline-variant/10"
              />
              <button 
                onClick={signOut}
                className="ml-2 p-2 rounded-full hover:bg-primary/5 text-primary/60 hover:text-primary transition-colors"
                title="Sign Out"
              >
